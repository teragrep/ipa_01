...
The environment variables can be passed directly on the container startup or by placing the link:https://www.freedesktop.org/software/systemd/man/latest/systemd.exec.html#EnvironmentFile=[EnvironmentFile] compatible files to `/ipa_01.conf.d/env/`.
...
services:
  ipa-server:
    volumes:
      - ./server.env:/ipa.conf.d/env/server.env
    env_file:
      - common.env
    environment:
